...
for s = 1:Ns    % loop through sites
    ...
    for y = 1:Ny        % loop through output dimensions
        ...
        ann{y,s,1} = trainANN(Xtrn,Ytrn(:,y),trainParms);
        
        % predict on segregated data
        Ztst(:,1) = ann{y,s,1}(Xtst');
...
%         
%         % train the network on segregated data
%         fprintf('Training Global ANN-WET: Site = %s; Target = %s ...',...
%             siteNames{s},targNames{y}); tic;
%         ann{y,s,2} = trainANN(Xwtrn,Ywtrn,trainParms);
%         Ztst(Iwtst,2) = ann{y,s,2}(Xwtst');
%         fprintf('. finished; time = %f \n',toc);
%         
%         % screen report
%         fprintf('Training Global ANN-DRY: Site = %s; Target = %s ...',...
%             siteNames{s},targNames{y}); tic;
%         ann{y,s,3} = trainANN(Xdtrn,Ydtrn,trainParms);
%         Ztst(Idtst,2) = ann{y,s,3}(Xdtst');
%         fprintf('. finished; time = %f \n',toc);
%         
        % store result
        Zsite(:,y,s,1) = Ztst(:,1);
        ...
        stats(y).site(s).ann(1) = calcStats(Ytst(:,y),Ztst(:,1),Bw);
        
%         % calculate ann-wetdry stats
%         stats(y).site(s).ann(2) = calcStats(Ytst,Ztst(:,2),Bw);
        
        % calculate model stats
        for m = 1:Nm
            stats(y).site(s).model(m) = calcStats(Ytst(:,y),Mtst(:,m),Bw);
        end
...
for y = 1:Ny
    ...
    % calculate ann-all stats
    stats(y).all.ann(1) = calcStats(Yall(:,y),Zall(:,y,1),Bw);
    ...
    % calculate improvement fractions
    fn = fieldnames(stats(y).all.ann(1));
    ratio = zeros(Nm+1,length(fn))/0;
    updn = [-1,1,1,-1,1,0,0,0,1];
%     ratio(1,1) = -(stats(y).all.ann(1).rmse - stats(y).all.ann(2).rmse) ...
%         ./ stats(y).all.ann(2).rmse;
%     ratio(1,2) = (stats(y).all.ann(1).mi - stats(y).all.ann(2).mi) ...
%         ./ stats(y).all.ann(2).mi;
    for m = 1:Nm
        for f = 1:length(fn)
            ratio(m+1,f) = updn(f)*(stats(y).all.ann(1).(fn{f}) - stats(y).all.model(m).(fn{f})) ...
                ./ stats(y).all.model(m).(fn{f});
            if updn(f) == 0
                ratio(m+1,f) = (abs(stats(y).all.ann(1).(fn{f})) - abs(stats(y).all.model(m).(fn{f}))) ...
