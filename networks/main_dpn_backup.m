...
for s = 3:Ns; tsite = tic;
    ...
    soilend = [];
    for a = 1:length(runs)
        if ~isnan(pals(first(A(a)),10))
            soilstart = first(A(a));
            soilend   = first(A(a)) + runs(A(a))-1;
            break
        end
    end
    assert(all(~isnan(pals(soilstart:soilend,10))));
...
%         
%         if Nsoil > 17520
%             [pals(soilstart:soilend,d),~] = ...
%                 grandma_smoothing(pals(soilstart:soilend,d),0.01);
%             if all(isnan(pals(soilstart:soilend,d)));
%                 fprintf('bad soils vector from grandma smoothing: %s. \n',varNames{d});
%             end
%             
%             for m = 1:Nm
%                 [model(soilstart:soilend,d,m),~] = ...
%                     grandma_smoothing(model(soilstart:soilend,d,m),0.01,0);
%             end
%             
%             if soilstart > 1
    ...
    for l = 1:Nl
        ...
        for y = Du+1:Dp
            ...
            for x = 1:Dp
                ...
                Bmin = min(min(Ytrn),min(Xtrn(:,y)))-1e-6; 
                Bmax = max(max(Ytrn),max(Xtrn(:,y)))+1e-6; 
                By = linspace(Bmin,Bmax,Nb);
                Bmin = min(Xtrn(:,x))-1e-6;                
                Bmax = max(Xtrn(:,x))+1e-6;                
                Bx = linspace(Bmin,Bmax,Nb);
                if x ~= y
                    [T(x,y,l,1,s),H(x,y,l,1,s),~] = ...
                        transfer_entropy(Ytrn,Xtrn(:,x),Xtrn(:,y),Bx,By);
                else
                    Yrnd = rand(size(Xtrn(:,y))) * ...
                        (max(Xtrn(:,y))-min(Xtrn(:,y))) + ...
                        min(Xtrn(:,y));
                    [T(x,y,l,1,s),H(x,y,l,1,s),~] = ...
                        transfer_entropy(Ytrn,Xtrn(:,x),Yrnd,Bx,By);
                end
                
                % perform on pals - with ann
                Bmin = min(min(Yann),min(Xtrn(:,y)))-1e-6; 
                Bmax = max(max(Yann),max(Xtrn(:,y)))+1e-6; 
                By = linspace(Bmin,Bmax,Nb);
                Bmin = min(Xtrn(:,x))-1e-6;                
                Bmax = max(Xtrn(:,x))+1e-6;                
                Bx = linspace(Bmin,Bmax,Nb);
                if x ~= y
                    [T(x,y,l,2,s),H(x,y,l,2,s),~] = ...
                        transfer_entropy(Yann,Xtrn(:,x),Xtrn(:,y),Bx,By);
                else
                    Yrnd = rand(size(Xtrn(:,y))) * ...
                        (max(Xtrn(:,y))-min(Xtrn(:,y))) + ...
                        min(Xtrn(:,y));
                    [T(x,y,l,2,s),H(x,y,l,2,s),~] = ...
                        transfer_entropy(Yann,Xtrn(:,x),Yrnd,Bx,By);
                end
                ...
                for m = 1:Nm; tmodel = tic;
                    ...
                    % lag data
                    X = window_average(model(:,x,m),lags(l));
                    if numel(X) < 1000; continue; end
                    Y = window_average(model(:,y,m),lags(l)); 
                    if numel(Y) < 1000; continue; end
                    ...
                    % calculate
                    Bmin = min(min(Ys),min(Yt))-1e-6; 
                    Bmax = max(max(Ys),max(Yt))+1e-6; 
                    By = linspace(Bmin,Bmax,Nb);
                    Bmin = min(Xt)-1e-6;              
                    Bmax = max(Xt)+1e-6;              
                    Bx = linspace(Bmin,Bmax,Nb);
                    if x ~= y
                        [T(x,y,l,2+m,s),H(x,y,l,2+m,s),~] = ...
                            transfer_entropy(Ys,Xt,Yt,Bx,By);
                    else
                        Yrnd = rand(size(Yt)) * (max(Yt)-min(Yt)) + min(Yt);
                        [T(x,y,l,2+m,s),H(x,y,l,2+m,s),~] = ...
                            transfer_entropy(Ys,Xt,Yrnd,Bx,By);
                    end
