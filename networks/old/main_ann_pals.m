...
%   if x == Du; continue; end;
   ...
   for y = Du+1:D
    ...
    % train the ANN and make predictions on a split-record set
    [Yann,Itrn,Itst] = ann_dpn_path(Ys,Xt,Yt);

    % do the actaul calculations
    Yann = Yann(:); Yann = Yann(Itst);
    Xt   = Xt(:);   Xt   = Xt(Itst);
    Yt   = Yt(:);   Yt   = Yt(Itst);
    [T(x,y),H(x,y),S(x,y)] = transfer_entropy_window_average(Yann,Xt,Yt,B(:,x),B(:,y));
